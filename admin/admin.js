  ...
  main.innerHTML =
    '<div class="p-6 max-w-2xl mx-auto">' +
      ...
      '<div class="rounded-xl p-5 mb-4" style="background:#1e293b;border:1px solid rgba(99,179,237,0.12)">' +
        ...
        // Rasm qismi
        '<label class="block text-xs font-semibold mb-2" style="color:#94a3b8;letter-spacing:1px">RASM (ixtiyoriy)</label>' +

        // Tab: URL yoki Fayl
        '<div class="flex gap-2 mb-3">' +
          '<button id="bcTabUrl" onclick="bcSwitchTab(&quot;url&quot;)" class="px-3 py-1 rounded text-xs font-semibold transition-all" ' +
            'style="background:#3b82f6;color:#fff">🔗 URL</button>' +
          '<button id="bcTabFile" onclick="bcSwitchTab(&quot;file&quot;)" class="px-3 py-1 rounded text-xs font-semibold transition-all" ' +
            'style="background:rgba(99,179,237,0.1);color:#64748b;border:1px solid rgba(99,179,237,0.15)">📁 Fayl</button>' +
        '</div>' +

        '<div id="bcUrlWrap">' +
          '<input id="bcImage" type="url" placeholder="https://... (rasm URL linki)" ' +
            'class="w-full rounded-lg px-3 py-2 text-sm" style="background:#0f172a;border:1px solid rgba(99,179,237,0.15);color:#f1f5f9;outline:none" ' +
            'oninput="bcPreviewUrl(this.value)">' +
        '</div>' +

        '<div id="bcFileWrap" style="display:none">' +
          '<label class="flex flex-col items-center justify-center w-full rounded-lg cursor-pointer" ' +
            'style="background:#0f172a;border:2px dashed rgba(99,179,237,0.2);padding:20px;transition:all .2s" ' +
            'onmouseover="this.style.borderColor=&quot;rgba(99,179,237,0.5)&quot;" onmouseout="this.style.borderColor=&quot;rgba(99,179,237,0.2)&quot;">' +
            '<span style="font-size:28px">🖼</span>' +
            '<span class="text-sm mt-1" style="color:#64748b">Rasm tanlash uchun bosing</span>' +
            '<span class="text-xs mt-1" style="color:#475569">JPG, PNG, WEBP — max 10MB</span>' +
            '<input id="bcFileInput" type="file" accept="image/*" style="display:none" onchange="bcPreviewFile(this)">' +
          '</label>' +
        '</div>' +

        '<div id="bcImagePreview" style="display:none;margin-top:10px">' +
          '<img id="bcPreviewImg" src="" style="max-height:180px;border-radius:8px;border:1px solid rgba(99,179,237,0.15)">' +
          '<button onclick="bcClearImage()" class="block mt-2 text-xs" style="color:#f87171">✕ Rasmni olib tashlash</button>' +
        '</div>' +
        ...
        '<div class="text-xs mt-1" style="color:#475569">HTML teglari ishlaydi: &lt;b&gt;kalın&lt;/b&gt;, &lt;i&gt;kursiv&lt;/i&gt;</div>' +
      '</div>' +

      '<div class="flex gap-3">' +
        '<button id="bcSendBtn" onclick="sendBroadcast()" class="w-full py-3 rounded-lg text-sm font-bold transition-all" ' +
          'style="background:linear-gradient(135deg,#3b82f6,#1d4ed8);color:#fff">📢 Barcha mijozlarga yuborish</button>' +
      '</div>' +
...
function bcSwitchTab(tab) {
  var urlWrap  = document.getElementById('bcUrlWrap');
  var fileWrap = document.getElementById('bcFileWrap');
  var tabUrl   = document.getElementById('bcTabUrl');
  var tabFile  = document.getElementById('bcTabFile');
  var activeStyle   = 'background:#3b82f6;color:#fff;border:none';
  var inactiveStyle = 'background:rgba(99,179,237,0.1);color:#64748b;border:1px solid rgba(99,179,237,0.15)';
  bcClearImage();
  if (tab === 'url') {
    urlWrap.style.display  = 'block';
    fileWrap.style.display = 'none';
    tabUrl.style.cssText  += activeStyle;
    tabFile.style.cssText += inactiveStyle;
  } else {
    urlWrap.style.display  = 'none';
    fileWrap.style.display = 'block';
    tabUrl.style.cssText  += inactiveStyle;
    tabFile.style.cssText += activeStyle;
  }
}

function bcPreviewUrl(url) {
...
function bcPreviewFile(input) {
  ...
  if (!file) return;
  if (file.size > 10 * 1024 * 1024) { alert('Fayl 10MB dan katta!'); input.value = ''; return; }
  var reader = new FileReader();
  reader.onload = function(e) {
    document.getElementById('bcPreviewImg').src = e.target.result;
    document.getElementById('bcImagePreview').style.display = 'block';
  };
  reader.readAsDataURL(file);
}

function bcClearImage() {
  var urlInput  = document.getElementById('bcImage');
  var fileInput = document.getElementById('bcFileInput');
  if (urlInput)  urlInput.value  = '';
  if (fileInput) fileInput.value = '';
  document.getElementById('bcImagePreview').style.display = 'none';
  document.getElementById('bcPreviewImg').src = '';
}

async function sendBroadcast() {
  var text      = document.getElementById('bcText').value.trim();
  var imgUrl    = document.getElementById('bcImage') ? document.getElementById('bcImage').value.trim() : '';
  var fileInput = document.getElementById('bcFileInput');
  ...
  if (fileInput && fileInput.files[0]) {
    var previewSrc = document.getElementById('bcPreviewImg').src;
    if (previewSrc && previewSrc.startsWith('data:')) imageBase64 = previewSrc;
  }

  if (!text && !imgUrl && !imageBase64) { alert('Matn yoki rasm kiriting'); return; }
  ...
  try {
    var d = await apiFetch('/admin/broadcast', {
      method: 'POST',
      body: JSON.stringify({ text: text, imageUrl: imgUrl, imageBase64: imageBase64 })
    });
    ...
    if (d.ok) {
      ...
      result.innerHTML = '✅ Yuborildi: <b>' + d.sent + '</b> ta &nbsp;|&nbsp; Xato: <b>' + d.failed + '</b> ta &nbsp;|&nbsp; Jami: <b>' + d.total + '</b> ta';
      // Formni tozalash
    ...
    btn.disabled    = false;
    btn.textContent = '📢 Barcha mijozlarga yuborish';
  }
}